...
public class SwaggerConfig {
	...
	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("Company API")
				.description("this api help to register and deleted the company details  API reference for developers")
	...
	public Docket postsApi() {
		...
		/*globalOperationParameters(Arrays.asList(new ParameterBuilder().name("").
		description("description of header").modelRef(new ModelRef("string")).parameterType("header").required(false).build()))*/
	}
}
